...
import { Calendar, Trophy, Coins, Gamepad2, AlertCircle } from "lucide-react";
...
function getTournamentDisplayName(tournament: Tournament) {
    const parts = [];
    ...
    if (tournament.serie?.full_name) {
        if (!tournament.league?.name || !tournament.serie.full_name.includes(tournament.league.name)) {
            ...
            // If serie name contains league name, use serie name as base?
            // Actually, usually Series Name = "League Name Season X"
            // So if we have League Name, we might duplicate.
            // Let's safe bet: add it. Users can request refinement.
            parts.push(tournament.serie.full_name);
        }
    }
    // Filter out parts that are already included?
    // Simple join is safer.
    // Refined logic:
    // 1. League Name
    // 2. Serie Full Name (if not same as league)
    // 3. Name (Tournoi/Phase)

    // Let's stick to the previous logic which was: League + Serie + Name
    // But dedupe simple strings.
    const cleanParts = [];
    if (tournament.league?.name) cleanParts.push(tournament.league.name);
    if (tournament.serie?.full_name && tournament.serie.full_name !== tournament.league?.name) {
        cleanParts.push(tournament.serie.full_name);
    }
    if (tournament.name) cleanParts.push(tournament.name);
...
export default function TournamentsClient({ initialData }: { initialData?: TournamentsData }) {
    const [selectedGame, setSelectedGame] = useState<VideoGameSlug>("cs-2");
    const [tournaments, setTournaments] = useState<TournamentsData | null>(initialData || null);
    const [loading, setLoading] = useState(!initialData);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        // If initialData is for the selected game, don't re-fetch
        if (initialData && selectedGame === "cs-2") { // Assuming initialData is always CS2 for now from server
            // But if user switches game, we MUST fetch.
            // Ideally we pass selectedGame as prop too if server rendering depends on query params.
            // For now, let's keep it simple: fetch on client side change.
        }
        ...
        if (initialData && selectedGame === "cs-2") {
            setLoading(false);
            // No need to fetch if it matches default.
            // However, ensuring correctness:
            return;
        }

        fetchTournaments();
    }, [selectedGame, initialData]);
